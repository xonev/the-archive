...
* [`src/finances_frontend/styles.cljs`](src/finances_frontend/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/finances_frontend/`](src/finances_frontend/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/finances_frontend/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `finances-frontend` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/finances_frontend/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns finances-frontend.example
  (:require [finances-frontend.config :as config])
